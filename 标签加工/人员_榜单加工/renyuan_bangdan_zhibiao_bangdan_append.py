...
# Description: 本程序主要计算榜单新增及变化，用榜单新增 和 用户全量表 join ,如果榜单表没有新增，则退出
# Question:
import sys
...
spark = SparkSession.builder \
    .appName("user_bd_zhibiao_bd_append") \
    .enableHiveSupport() \
    .config(conf=conf) \
    .getOrCreate()
...
for tag_code in tag_codes:
    ...
    try:
        ...
        joined_df.createOrReplaceTempView("hang_ye_dai_biao")
        ...
        spark.sql("""
              ...
              (
                   ...
                   ( -- 如果 xm 不为 null 
                        select * 
                        from hang_ye_dai_biao
                        where xm is not null
                   ) tt1 
                   join tmp_lk.tmp_person_info tt2 
                   on tt1.xm=tt2.person_name and tt1.qymc=tt2.enterprise_name -- 通过 姓名+企业编码找到对应的人
                   union all 
                   select tt2.person_hkey,
                         tt2.person_name as nonpublic_name,
                         current_timestamp() as tag_time, -- 标签产生时间
                   ...
                   join tmp_lk.tmp_person_info tt2 
                   on tt1.qymc=tt2.enterprise_name -- 通过 企业编码 给这个企业所有的人都搭上我标签 
              ) t 
